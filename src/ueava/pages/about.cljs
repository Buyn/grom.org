...
(defn contact-card [href title value icon-svg icon-bg-class hover-color-class]
  [:a {:href href
       :target "_blank"
   ...
   [:div {:class (str "w-14 h-14 rounded-full flex items-center justify-center transition-colors duration-300 " icon-bg-class)}
    icon-svg]
   [:div {:class "text-center"}
    [:span {:class "block text-sm text-gray-400 font-medium mb-1"} title]
    [:span {:class (str "text-lg font-semibold text-gray-800 transition-colors " hover-color-class)} value]]])
  ...
  [:section {:class "py-20 bg-gray-50" :id "contacts"}
   [:div {:class "max-w-4xl mx-auto px-8 text-center"}
    ...
    [:div
     ...
     ;; Email Card
     [contact-card
      "mailto:ueavaorg@gmail.com"
      (tr :email)
      "ueavaorg@gmail.com"
      [:svg {:class "w-7 h-7" :fill "none" :view-box "0 0 24 24" :stroke "currentColor"}
       [:path {:stroke-linecap "round" :stroke-linejoin "round" :stroke-width "2"
               :d "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"}]]
      "bg-ueava-brown/10 text-ueava-brown group-hover:bg-ueava-brown group-hover:text-white"
      "group-hover:text-ueava-brown"]

     ;; Instagram Card
     [contact-card
      "https://www.instagram.com/_ueava_"
      (tr :instagram)
      "@_ueava_"
      [:svg {:class "w-7 h-7" :fill "currentColor" :view-box "0 0 24 24"}
      ...
      "bg-pink-500/10 text-pink-600 group-hover:bg-pink-600 group-hover:text-white"
